...
class Conexion {
	...
	Statement stment
	ResultSet rs
	RepositorioReceta repoReceta = RepositorioReceta.instance
	...
	def obtenerRecetas() {
		val sql = "select * from receta;"
		val rs = this.ejecutarQuery(sql)

		while (rs.next()) {
			repoReceta.update(new Receta() => [
				nueva = false
	...
	def insertarReceta(Receta receta) {
		...
		sql.append("'" + receta.descripcion + "','" + receta.nombre + "' ); ")
		this.ejecutarUpdate(sql.toString)
		receta.articulos.forEach [ art |
			var String str
			str = "insert into compone values(" + art.idArticulo.toString() + "," +
				receta.idReceta.toString() + "); "
			this.ejecutarUpdate(str)
